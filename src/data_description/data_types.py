...
# from src.db_connect.db_engine import Engine, Read_Write
...
class DataDescribe:
    ...
    def fetch_attr_dtype(self, file_path, user_id, session_id, sheet_name=0, connection=None):
        ...
        if df_ is not None and len(df_) > 0:
            msg = {
                'msg': f'''file is already present for this user_id: {user_id} and session_id: {session_id}.Do you want to update that file'''}
        ...
        try:
            ...
            print("Map Attr", map_attribute)

            try:
                dest_file_path = os.path.join(self.std_all_file, new_filename)
                shutil.copy(new_path, dest_file_path)

                parquet_file_path = os.path.join(self.std_parquet_dir,
                                                 f"{user_id}_{session_id}_{original_name}.parquet")
                ...
                df[str_cols.columns] = str_cols.apply(
            ...
            try:
                ...
                unique_df.to_parquet(output_path, engine='pyarrow')
            except:
            ...
            for k, v in update_dtype.items():
                try:
                    if v.__contains__('date'):
                        ...
                        updated_df[k] = pd.to_datetime(updated_df[k], errors='coerce',
    ...
    def replace_file_attributes(self, file_path, user_id, session_id, sheet_name=0, connection=None):
        try:
            # connection= self.connect.connect_engine()
            # print(connection)
            move_file = self.update_file.move_files(user_id, session_id, self.std_all_file, self.std_parquet_dir,
                                                    self.archive_all_file, self.archive_parquet_file)
            ...
            print("this is  digvijay sir ")
            dest_file_path = os.path.join(self.std_all_file, new_filename)
            print(dest_file_path, new_path)
            shutil.copy(new_path, dest_file_path)
            print("file copied", new_path, dest_file_path)
            ...
            try:
                ...
                pass
            save_parquet = df.to_parquet(parquet_file_path, engine='pyarrow', index=False)
            print("paraquet file saved")

...
from be_src.tatva_util.tatvaAi_utils import Tatva_Utils
from be_src.db_connection.db_engine import Engine, Read_Write
from be_src.config.config import Config
...
class TatvaAIMain():
    ...
    def get_parquet_path(self, base_dir, user_id,session_id,file_name):
        parquet_path = os.path.join(base_dir, f"{user_id}_{session_id}_{file_name}.paraquet")
        if not os.path.exists(parquet_path):
            raise FileNotFoundError(f"File not found: {parquet_path}")
        return parquet_path

    def query_analysis(self, userid, session_id, question, file_name):
        print(userid, session_id, question, file_name)
        parquet_path = self.get_parquet_path(self.base_dir, userid, session_id,file_name )
        print(parquet_path)
